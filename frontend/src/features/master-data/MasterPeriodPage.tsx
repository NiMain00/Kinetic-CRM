...
export default function MasterPeriodPage() {
  ...
  useEffect(() => { fetchEntity('periods'); }, [fetchEntity]);
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (editing) {
      ...
      const id = crypto.randomUUID?.() || `PER-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`;
    ...
    <div className="flex-1 flex flex-col h-full bg-background overflow-hidden text-on-surface">
      <div className="bg-surface-container-lowest border-b border-border px-3 sm:px-6 lg:px-8 py-3 sm:py-4 shrink-0 flex flex-col sm:flex-row justify-between sm:items-center gap-4 shadow-sm z-10">
        <div>
          ...
          <p className="text-[11px] text-outline mt-0.5">Kelola periode pelaporan dan anggaran.</p>
        </div>
        ...
        <div className="max-w-7xl mx-auto space-y-6 text-left">
          <Card padding="md">
            <div className="relative">
              ...
              <input type="text" placeholder="Cari periode..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-8 pr-3 py-2 bg-surface-container-low border border-border rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-primary" aria-label="Cari periode" />
            </div>
          </Card>
            ...
            <div className="overflow-x-auto scrollbar-none table-mobile-compact">
              <table className="w-full text-xs text-left table-auto" role="table" aria-label="Daftar Periode">
                ...
                <tbody className="divide-y divide-border">
                  {filtered.length === 0 ? (
                    <tr><td colSpan={5} className="px-6 py-12 text-center text-outline italic">Tidak ada periode ditemukan.</td></tr>
                  ) : (
                    filtered.map(p => (
                      <tr key={p.id} className="hover:bg-surface-container-low/65 transition-colors">
                        ...
                        <td className="px-6 py-4 text-on-surface-variant">{formatDate(p.start_date)}</td>
                        <td className="px-6 py-4 text-on-surface-variant">{formatDate(p.end_date)}</td>
                        <td className="px-6 py-4 text-center">
                          <button onClick={() => toggleStatus(p.id)} className={`inline-flex items-center justify-center p-0.5 rounded-full w-9 h-5 transition-colors outline-none cursor-pointer ${p.is_active ? 'bg-success' : 'bg-surface-container-highest'}`} aria-label={`Toggle status ${p.name}`}>
                            <span className={`w-4 h-4 bg-surface-container-lowest rounded-full shadow-xs transform transition-transform duration-200 ${p.is_active ? 'translate-x-2' : '-translate-x-2'}`} />
                          </button>
                        </td>
                        <td className="px-6 py-4 text-right">
                          <div className="flex gap-1 justify-end">
                            <button onClick={() => openEdit(p)} className="p-1.5 rounded-lg hover:bg-surface-container text-outline hover:text-primary transition-colors cursor-pointer" title="Edit"><span className="material-symbols-outlined icon-compact text-[18px]">edit</span></button>
                            <button onClick={() => handleDelete(p.id)} className="p-1.5 rounded-lg hover:bg-danger-container dark:bg-danger-container/30 text-outline hover:text-danger transition-colors cursor-pointer" title="Hapus"><span className="material-symbols-outlined icon-compact text-[18px]">delete</span></button>
                          </div>
                        </td>
                      </tr>
                    ))
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs z-50 flex justify-end animate-fade-in">
          <div className="w-full max-w-lg bg-surface-container-lowest h-full shadow-2xl flex flex-col">
            <div className="p-6 border-b border-border bg-surface-container-low flex items-center justify-between">
              <div>
                <h3 className="font-display-title text-sm font-extrabold text-on-surface">{editing ? 'Edit Periode' : 'Tambah Periode Baru'}</h3>
                <p className="text-[10px] text-outline mt-1">{editing ? `ID: ${editing.id}` : 'Buat periode baru'}</p>
              </div>
            ...
            <form onSubmit={handleSave} className="flex flex-col h-full">
              <div className="p-6 flex-1 overflow-y-auto space-y-5 text-left text-xs">
                <div className="space-y-2">
                  ...
                  <input id="period-name" type="text" value={form.name || ''} onChange={e => setForm({ ...form, name: e.target.value })} className="w-full rounded-lg border border-border p-2.5 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-xs" placeholder="Contoh: 2026 Q1" required />
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label htmlFor="period-start" className="font-semibold text-on-surface block">Tanggal Mulai *</label>
                    <input id="period-start" type="date" value={form.start_date || ''} onChange={e => setForm({ ...form, start_date: e.target.value })} className="w-full rounded-lg border border-border p-2.5 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-xs" required />
                  </div>
                  <div className="space-y-2">
                    <label htmlFor="period-end" className="font-semibold text-on-surface block">Tanggal Selesai *</label>
                    <input id="period-end" type="date" value={form.end_date || ''} onChange={e => setForm({ ...form, end_date: e.target.value })} className="w-full rounded-lg border border-border p-2.5 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-xs" required />
                  </div>
                </div>
                <div className="space-y-2">
