        ...
        <div className="max-w-5xl mx-auto space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
            <div className="bg-surface-container-lowest border border-border p-4 rounded-xl shadow-sm">
              <p className="text-[10px] text-outline uppercase font-mono tracking-wider">Total Konektor</p>
              <p className="text-xl font-extrabold text-on-surface mt-1">{connectors.length}</p>
            </div>
            <div className="bg-surface-container-lowest border border-border p-4 rounded-xl shadow-sm">
            ...
            <div className="bg-surface-container-lowest border border-border p-4 rounded-xl shadow-sm">
              <p className="text-[10px] text-outline uppercase font-mono tracking-wider">Tidak Aktif</p>
              <p className="text-xl font-extrabold text-warning mt-1">{connectors.filter(c => !c.active).length}</p>
            </div>
            <div className="bg-surface-container-lowest border border-border p-4 rounded-xl shadow-sm">
              <p className="text-[10px] text-outline uppercase font-mono tracking-wider">Error</p>
                ...
                <tbody className="divide-y divide-border">
                  {connectors.map((c) => (
                    <tr key={c.id} className="hover:bg-surface-container-low/65 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-outline">{c.description}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className="px-2 py-0.5 bg-surface-container text-on-surface-variant rounded text-[10px] font-semibold badge-compact">{c.type}</span>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-outline text-[10px] font-mono">{c.lastTested || '-'}</td>
