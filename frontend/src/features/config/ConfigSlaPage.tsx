      ...
      <div className="flex-1 overflow-y-auto p-6 sm:p-8 custom-scrollbar">
        <div className="max-w-5xl mx-auto space-y-6 text-left">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-surface-container-lowest border border-border p-4 rounded-xl shadow-xs"><p className="text-[10px] text-outline uppercase font-mono tracking-wider">Rata-rata Ambang Peringatan</p><p className="text-xl font-extrabold text-warning mt-1">{Math.round(configs.reduce((s, c) => s + c.warningThreshold, 0) / configs.length)} {configs[0]?.unit}</p></div>
            <div className="bg-surface-container-lowest border border-border p-4 rounded-xl shadow-xs"><p className="text-[10px] text-outline uppercase font-mono tracking-wider">Rata-rata Ambang Kritis</p><p className="text-xl font-extrabold text-danger mt-1">{Math.round(configs.reduce((s, c) => s + c.criticalThreshold, 0) / configs.length)} {configs[0]?.unit}</p></div>
          </div>
