...
import type { Project, ApprovalItem } from '../../types/domain';
...
export default function DashboardView({
  ...
  useEffect(() => {
    ...
    setCurrentDateString(today.toLocaleDateString('id-ID', options));
  }, []);
    ...
    <div className="p-8 space-y-8 overflow-y-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-secondary font-body-main mt-1 italic" id="current-date">
            {currentDateString}
          </p>
        </div>
        <div className="flex gap-3">
          <button
            onClick={() => onNavigatePage('prospects')}
            className="flex items-center gap-2 bg-white border border-border px-4 py-2.5 rounded-lg font-label-sm text-label-sm hover:bg-surface-container-low transition-all"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 bg-primary text-on-primary px-4 py-2.5 rounded-lg font-label-sm text-on-primary hover:bg-primary-container transition-all shadow-sm"
          >
      ...
      {/* Stats Summary Cards (Bento Style Row) */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white border border-border p-6 rounded-xl shadow-sm flex flex-col justify-between group hover:border-primary transition-all duration-300">
          ...
          <div>
            <p className="text-secondary font-label-sm mb-1 text-sm bg-transparent">Total Proyek Aktif</p>
            <h3 className="font-display-title text-display-title text-on-surface text-xl">Rp 42.8B</h3>
          </div>
        </div>

        <div className="bg-white border border-border p-6 rounded-xl shadow-sm flex flex-col justify-between group hover:border-status-orange transition-all duration-300">
          <div className="flex justify-between items-start mb-4">
            <span className="p-3 rounded-lg bg-status-orange/10 text-status-orange">
              <span className="material-symbols-outlined">fact_check</span>
            </span>
            <span className="text-status-orange font-label-sm text-xs">High Priority</span>
          </div>
          <div>
            <p className="text-secondary font-label-sm mb-1 text-sm">Approval Pending</p>
            <h3 className="font-display-title text-display-title text-on-surface text-xl">24 Items</h3>
          </div>
        </div>

        <div className="bg-white border border-border p-6 rounded-xl shadow-sm flex flex-col justify-between group hover:border-danger transition-all duration-300">
          <div className="flex justify-between items-start mb-4">
            <span className="p-3 rounded-lg bg-danger/10 text-danger">
              <span className="material-symbols-outlined">alarm</span>
            </span>
            <span className="text-danger font-label-sm text-xs">Due &lt; 7 Days</span>
          </div>
          <div>
            <p className="text-secondary font-label-sm mb-1 text-sm">Approaching Deadline</p>
            <h3 className="font-display-title text-display-title text-on-surface text-xl">08 Proyek</h3>
          </div>
        </div>

        <div className="bg-white border border-border p-6 rounded-xl shadow-sm flex flex-col justify-between group hover:border-status-indigo transition-all duration-300">
          <div className="flex justify-between items-start mb-4">
            <span className="p-3 rounded-lg bg-status-indigo/10 text-status-indigo">
              <span className="material-symbols-outlined">military_tech</span>
            </span>
            <span className="text-status-indigo font-label-sm text-xs">YTD Performance</span>
          </div>
          <div>
            <p className="text-secondary font-label-sm mb-1 text-sm">Win Rate %</p>
            <h3 className="font-display-title text-display-title text-on-surface text-xl">68.4%</h3>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-12 gap-6">
        {/* Chart Widget: Trend Win/Loss 6 Bulan */}
        <div className="col-span-12 lg:col-span-8 bg-white rounded-xl border border-border shadow-sm p-6 overflow-hidden">
          <div className="flex justify-between items-center mb-8">
            <div>
              <h4 className="font-heading-section text-heading-section">Trend Win/Loss</h4>
              <p className="text-secondary font-caption-xs text-xs">Performance trajectory over the last 6 months</p>
            </div>
        ...
        {/* Status Widget (Donut Chart representation) */}
        <div className="col-span-12 lg:col-span-4 bg-white rounded-xl border border-border shadow-sm p-6 flex flex-col justify-between">
          ...
          <div className="relative flex justify-center items-center py-6">
            <svg className="w-44 h-44 -rotate-90">
              ...
              {/* Primary (In Progress) circle sector */}
            ...
            <div className="absolute text-center">
              <p className="font-display-title text-display-title text-on-surface text-xl">142</p>
          ...
          <div className="space-y-2 mt-2">
            <div className="flex justify-between items-center text-sm">
              <div className="flex items-center gap-2">
                ...
                <span className="text-secondary font-label-sm">In Progress</span>
              </div>
              <span className="font-mono-data text-mono-data font-bold">72</span>
            </div>
            <div className="flex justify-between items-center text-sm">
              <div className="flex items-center gap-2">
              ...
              <span className="font-mono-data text-mono-data font-bold">45</span>
            </div>
            <div className="flex justify-between items-center text-sm">
              <div className="flex items-center gap-2">
                <span className="w-2.5 h-2.5 rounded-full bg-secondary-container"></span>
                <span className="text-secondary font-label-sm">On Hold</span>
              </div>
              <span className="font-mono-data text-mono-data font-bold">25</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Tables Grid Layout */}
      <div className="grid grid-cols-12 gap-6">
        {/* List Widget: Approval Pending */}
        <div className="col-span-12 lg:col-span-7 bg-white rounded-xl border border-border shadow-sm overflow-hidden">
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-surface-container-low text-secondary font-caption-xs uppercase border-b border-border">
                <tr>
                  ...
                  <th className="px-6 py-3 font-semibold text-xs">Waiting</th>
                  <th className="px-6 py-3 font-semibold text-right text-xs">Action</th>
                </tr>
              </thead>
        ...
        {/* Alert List: Approaching Deadline */}
        <div className="col-span-12 lg:col-span-5 bg-white rounded-xl border border-border shadow-sm p-6 flex flex-col justify-between">
          <div className="flex justify-between items-center mb-6">
            <h4 className="font-heading-section text-heading-section">Critical Deadlines</h4>
