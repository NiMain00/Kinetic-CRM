...
export default function OverviewTab({ project: propProject }: TabProps) {
  const project = propProject || INITIAL_PROJECTS[0];

  const teamMembers = [
    { initials: 'BS', name: 'Bambang Sutejo', role: 'Project Manager' },
    { initials: 'RA', name: 'Rina Amalia', role: 'Site Engineer' },
    { initials: 'DS', name: 'Deni Saputra', role: 'Procurement' },
    { initials: 'SM', name: 'Siti Mulyani', role: 'Finance' },
  ];
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div className="md:col-span-2 space-y-6">
        <Card padding="lg">
          ...
          <div className="grid grid-cols-2 gap-y-4 text-sm">
            ...
            <div>
              ...
              <p className="font-semibold text-primary text-base">{formatCurrency(project.estimatedValue)}</p>
            </div>
            <div>
        ...
        <Card padding="lg" className="bg-primary-container text-on-primary-container overflow-hidden relative">
          <div className="relative z-10">
            ...
            <div className="w-full bg-white/20 h-2.5 rounded-full overflow-hidden">
              <div className="bg-white h-full rounded-full" style={{ width: `${project.progress}%` }}></div>
            </div>
            <p className="mt-4 font-caption-xs text-white/70 text-xs">
              {project.progress < 100 ? 'Lanjut ke tahap berikutnya setelah persetujuan' : 'Proyek selesai'}
            </p>
          </div>
        ...
        <Card padding="lg">
          <h3 className="font-heading-section text-heading-section mb-4">Tim Proyek</h3>
          <div className="space-y-3">
            {teamMembers.map((member) => (
              <div key={member.name} className="flex items-center gap-3 p-2 rounded-lg hover:bg-surface-container-low transition-colors">
                <span className="w-9 h-9 rounded-full bg-primary/10 text-primary text-xs font-bold flex items-center justify-center">
                  {member.initials}
                </span>
                <div>
        ...
        <Card padding="lg">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-3 bg-surface-container-low rounded-lg">
              ...
              <span className="font-bold text-on-surface">{formatCurrency(project.estimatedValue)}</span>
            </div>
            <div className="flex items-center justify-between p-3 bg-surface-container-low rounded-lg">
