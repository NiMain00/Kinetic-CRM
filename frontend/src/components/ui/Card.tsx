...
export default function Card({ children, className = '', padding = 'md', header, footer, hover = false }: CardProps) {
  ...
  return (
    <div className={`${hasBg ? '' : 'bg-surface'} rounded-2xl border border-border/60 shadow-card ${hover ? 'hover:shadow-card-hover transition-all duration-200 hover:-translate-y-0.5' : ''} ${className}`}>
