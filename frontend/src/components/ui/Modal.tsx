      ...
      <div ref={focusTrapRef} className={`bg-surface sm:rounded-2xl shadow-modal w-full ${sizes[size]} max-h-[95vh] sm:max-h-[90vh] flex flex-col animate-in zoom-in-95 fade-in duration-200 rounded-t-2xl sm:rounded-2xl`}>
        {title && (
          <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-border/60">
            ...
            <button onClick={onClose} className="w-9 h-9 sm:w-8 sm:h-8 rounded-xl flex items-center justify-center text-outline hover:bg-surface-container hover:text-on-surface transition-colors touch-min" aria-label="Tutup dialog">
              <span className="material-symbols-outlined text-lg">close</span>
            </button>
          </div>
        )}
        <div className="flex-1 overflow-y-auto p-4 sm:p-6">{children}</div>
        {footer && <div className="px-4 sm:px-6 py-3 sm:py-4 border-t border-border/60 bg-surface-container-low flex justify-end gap-3">{footer}</div>}
      </div>
    </div>
  );
}
